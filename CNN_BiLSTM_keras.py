...
Data_all = (Amazon_Echo_numpy, Belkin_wemo_motion_sensor_numpy, Belkin_Wemo_switch_numpy, HP_Printer_numpy,
            ...
            Withings_Smart_Baby_Monitor_numpy, Blipcare_Blood_Pressure_meter_numpy, Dropcam_numpy, iHome_numpy, Nest_Dropcam_numpy, NEST_Protect_smoke_alarm_numpy,
...
print(f"total samples:{len(data_train) + len(data_test)}")
# ============================================================================
...
dropout1 = 0.11245974166556161
dropout2 = 0.3207527760045966
dropout3 = 0.7342146978592597
dropout4 = 0.692539034315719
dropout5 = 0.21280043312755825
...
model.summary()
#编译模型
# optimizers = SGD(lr=0.01, decay=1e-6, momentum=0.9, nesterov=True) # 训练最高0.9564
...
history = model.fit(X_train[5000:], Y_train[5000:], batch_size=300, epochs=50, validation_data=(X_validation, Y_validation), verbose=1) # verbose = 2 为每个epoch输出一行记录
#评估模型
...
plot_confusion(model, X_test, Y_test) # 调用 plot_confuse 函数
